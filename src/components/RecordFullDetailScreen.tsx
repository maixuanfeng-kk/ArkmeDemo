...
type RecordFullDetailScreenProps = {
  ...
  onRecognizeArrangement?: (record: RecordItem) => void;
};
    ...
    <div className="flex h-full flex-col bg-bg">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <MainRecordCard
          ...
          getRecognizedArrangement={getRecognizedArrangement}
          onRecognizeArrangement={onRecognizeArrangement}
...
function MainRecordCard({
  ...
  const canRecognizeArrangement =
    canRecognizeRecordAsArrangement(record) && Boolean(onRecognizeArrangement);
  const recognizedArrangement = canRecognizeArrangement
    ? getRecognizedArrangement?.(record.uid)
    : null;
  const arrangementActionLabel = recognizedArrangement
    ...
    <article className="w-full rounded-b-[8px] border-b border-[var(--record-detail-main-border)] bg-[var(--record-detail-main-bg)]">
      <div className="relative px-4 pb-4 pt-4">
        ...
        {canRecognizeArrangement && (
          <button
            type="button"
            className="mt-3 inline-flex items-center justify-center gap-1.5 rounded-[8px] border border-primary/20 bg-primary-soft px-3 py-2 text-[13px] font-semibold leading-5 text-primary transition active:scale-[0.98]"
            onClick={() => onRecognizeArrangement?.(record)}
          >
            <svg
              className="h-4 w-4"
              viewBox="0 0 24 24"
