...
import ArrangementSourceRecognitionDialog from "@/components/arrangements/ArrangementSourceRecognitionDialog";
import { canRecognizeRecordAsArrangement } from "@/components/arrangements/recordArrangementSource";
import Arrangements from "@/pages/Arrangements";
import Records from "@/pages/Records";
import { aiConversationLogEntries } from "@/data/aiConversationLog";
import {
  findArrangementBySourceRecordUid,
  setArrangementOpenTargetId,
...
export default function Home({ currentPage, onNavigate }: HomeProps) {
  ...
  const [sourceRecognitionRecord, setSourceRecognitionRecord] =
  ...
  const sourceRecognitionContextRecords = React.useMemo<RecordItem[]>(
    () => [...selfRecords, ...testConversationRecords],
    [selfRecords, testConversationRecords]
  );
  ...
  const openArrangementDetail = React.useCallback((arrangement: ArrangementItem) => {
    setArrangementOpenTargetId(arrangement.id);
    setSourceRecognitionRecord(null);
  ...
  const openSourceRecordRecognition = React.useCallback((record: RecordItem) => {
    const existingArrangement = findArrangementBySourceRecordUid(record.uid);
    if (existingArrangement) {
      openArrangementDetail(existingArrangement);
      return;
    }

    setSourceRecognitionRecord({
      ...record,
      sourceConversation: record.sourceConversation ?? makeSelfSource(record.uid),
    });
  }, [makeSelfSource, openArrangementDetail]);

  const finishSourceArrangementCreation = React.useCallback(
    (arrangement: ArrangementItem) => {
      openArrangementDetail(arrangement);
    },
    [openArrangementDetail]
  );

  const getSourceRecordArrangement = React.useCallback(
    (recordUid: string) => findArrangementBySourceRecordUid(recordUid),
    []
  );

  const openArrangementsFromSourceDialog = React.useCallback(() => {
    setShowSendToSelf(false);
    setShowTestConversation(false);
    setShowSearch(false);
    onNavigate("arrangements");
    ...
    if (recordDetail) {
      return (
        <RecordFullDetailScreen
          ...
          getRecognizedArrangement={getSourceRecordArrangement}
          onRecognizeArrangement={openSourceRecordRecognition}
        />
      );
    }
    ...
    if (showSendToSelf) {
      return (
        <SendToSelfConversationChat
          ...
          getRecognizedArrangement={getSourceRecordArrangement}
          onRecognizeRecord={openSourceRecordRecognition}
          onOpenRecordDetail={setRecordDetail}
          onOpenRecordSnapshot={setRecordSnapshot}
        />
      );
      ...
      mainPane={
        <div className="relative flex min-h-0 flex-1 flex-col">
          ...
          <RecordDetailSheet
            ...
            onRecognizeArrangement={openSourceRecordRecognition}
          />
          <ArrangementSourceRecognitionDialog
            record={sourceRecognitionRecord}
            contextRecords={sourceRecognitionContextRecords}
            locale={resolvedLocale}
            onClose={() => setSourceRecognitionRecord(null)}
            onCreated={finishSourceArrangementCreation}
            onOpenArrangements={openArrangementsFromSourceDialog}
          />
        </div>
      }
    />
...
function SendToSelfConversationChat({
  ...
  getRecognizedArrangement: (recordUid: string) => ArrangementItem | null;
  ...
  return (
    <div className="flex h-full flex-col bg-bg">
      ...
      <ChatList
        ...
        getRecordInlineAction={(record) => {
          const existingArrangement = getRecognizedArrangement(record.uid);
          return {
            label: existingArrangement
              ? t("arrangements.ai.viewCreatedArrangement")
              : t("arrangements.ai.recognizeFromRecord"),
      ...
      >
        <div className="space-y-3">
          {sortedRecords.map((record, index) => {
            ...
            const canRecognizeArrangement = canRecognizeRecordAsArrangement(record);
            const existingArrangement = canRecognizeArrangement
              ? getRecognizedArrangement(record.uid)
              : null;
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 max-w-[82%]">
                      ...
                      {canRecognizeArrangement && (
                        <button
                          type="button"
                          className="mt-2 inline-flex min-h-7 max-w-full items-center gap-1.5 rounded-[8px] border border-primary/20 bg-primary-soft px-2 py-1 text-left text-[12px] font-semibold leading-4 text-primary transition hover:bg-primary-soft/80 active:scale-[0.98]"
                          onClick={() => onRecognizeRecord(record)}
                        >
