...
当前仓库是候选人maixuanfeng-kk的交付版本，已经围绕 README 中的“安排”模块原始需求完成了多轮最小可用迭代，重点工作如下：

1. 完成了“安排”模块的基础闭环：
   - 新增安排入口、列表、详情、手动创建。
   - 支持待处理、以后再说、已完成三种状态。
   - 关键状态切换和删除操作使用居中确认弹窗，降低误触。

2. 完成了 AI 绑定与明确文本识别：
   - 支持填写 OpenAI-compatible 的 Base URL 和 API Key。
   - 支持从服务端拉取可用模型列表后选择模型。
   - 明确文本识别后不会直接入库，而是先进入“待确认创建”。
   - 处理了浏览器直连 CORS 问题，补了本地代理链路。

3. 完成了多来源识别场景：
   - 支持从“发给自己”识别安排。
   - 支持从私聊识别安排。
   - 支持从群聊识别安排。
   - 已创建来源可直接“查看安排”，避免重复创建。

4. 完成了智能归并与上下文脉络：
   - 同一件事来自多条消息时可归并到同一条安排。
   - 详情页以时间线展示关联来源、原文、识别理由和上下文。
   - 连续对话识别会带入最近多轮上下文，而不是只看单条消息。
   - 修复并收敛了标题、相关人、地点等字段边界问题。

5. 完成了时间总览与提醒机制：
   - 支持截止时间、时间段、提醒时间。
   - 新增列表 / 日历双视图。
   - 支持提前提醒、循环提醒、提醒后“稍后再看 / 已看过”等低焦虑处理。
   - 手动创建里的日期和时间改为选择控件，不再依赖手填自然语言。

6. 完成了移动端视觉重构：
   - 将页面从通用任务管理样式，调整为更有“私人感”的移动端安排本。
   - 强化时间、相关人、当天日期等核心信息的视觉层级。
   - 保持界面简约、克制，不改动既有业务逻辑。

7. 完成了项目工程约定与提交流程补强：
   - 新增 `docs/project-conventions.md`，明确代码逻辑清晰度、文件/函数行数约束、字段边界和移动端交互约定。
   - 补齐候选人日志与应用内 AI 对话日志。
   - 修复本仓库 Windows 环境下的 `pre-push` 校验执行链路，确保 `git push` 前可自动跑 `verify:answer`。

## 本次自测说明

- 已多次运行 `pnpm lint`、`pnpm build`、`pnpm verify:answer`。
- 已在移动端 Demo `http://127.0.0.1:5173/` 进行功能验证。
- 已在消息测试后台 `http://127.0.0.1:5173/sendtest` 验证私聊 / 群聊 / 发给自己相关链路。
- AI 识别链路已通过本地代理与兼容接口联调验证，待确认创建流程已跑通。
